...
fn validate_create_entry_edge(validate_data: ValidateData) -> ExternResult<ValidateCallbackResult> {
    ...
    if let Header::Create(_) = validate_data.element.header().clone() {
        let _parent_goal: Goal = match resolve_dependency(proposed_edge.parent_address.0.into())? {
        ...
        let _child_goal: Goal = match resolve_dependency(proposed_edge.child_address.0.into())? {
            Ok(ResolvedDependency(_, child_goal)) => child_goal,
            // we want to forward the validate_callback_result
            // back to Holochain since it contains a specific UnresolvedDependencies response
            // including the missing Hashes
            Err(validate_callback_result) => return Ok(validate_callback_result),
